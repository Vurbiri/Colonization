...
namespace Vurbiri
{
    public static class Storage
    {
        public static IEnumerator TryLoadTextureWeb_Cn(string url, Action<Return<Texture>> callback)
        {
            ...
            {
                Log.Warning("==== UnityWebRequest: " + request.error);
                callback?.Invoke(Return<Texture>.Empty);
                yield break;
            }
        ...
        public static bool TryLoadObjectFromResourceJson<T>(string path, out T obj, bool logWarning = true)
        {
            try
            {
            ...
            {
                if(logWarning) Log.Warning($"[Storage] Не удалось загрузить объект {typeof(T).Name} по пути {path}.\n".Concat(ex.Message));
                obj = default;
                return false;
            }
        }
