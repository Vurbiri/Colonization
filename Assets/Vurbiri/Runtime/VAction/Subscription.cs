using System;
using Impl = System.Runtime.CompilerServices.MethodImplAttribute;

namespace Vurbiri
{
	public interface IUnsubscribed<in TDelegate> where TDelegate : Delegate
	{
		public void Remove(TDelegate action);
	}

	public class Subscription : IDisposable
	{
		private Action _action;

		public static readonly Subscription Empty = new(Dummy.Action);

        [Impl(256)] private Subscription(Action action) => _action = action;

		[Impl(256)] public static Subscription Create<T>(IUnsubscribed<T> subscriber, T action) where T : Delegate
		{
			var unsub = new Unsubscribe<T>(subscriber, action);
			return new(unsub.Invoke);
		}

		[Impl(256)] public void Dispose()
		{
			_action(); 
			_action = Dummy.Action;
		}

        [Impl(256)] public static Subscription operator +(Subscription a, Subscription b) => new((a?._action + b?._action) ?? Dummy.Action);
		[Impl(256)] public static Subscription operator ^(Subscription a, Subscription b)
		{
			a?._action();
			return b;
		}

		// ******************** Nested ********************************
		private class Unsubscribe<TDelegate> where TDelegate : Delegate
		{
			private WeakReference<IUnsubscribed<TDelegate>> _weakSigner;
			private TDelegate _action;

			[Impl(256)] public Unsubscribe(IUnsubscribed<TDelegate> subscriber, TDelegate action)
			{
				_weakSigner = new(subscriber);
				_action = action;
			}

			public void Invoke()
			{
				if (_weakSigner != null && _weakSigner.TryGetTarget(out IUnsubscribed<TDelegate> signer))
				{
					signer.Remove(_action);

					_weakSigner.SetTarget(null);
					_weakSigner = null;
				}
				_action = null;
			}
		}
		// ************************************************************
	}
}
