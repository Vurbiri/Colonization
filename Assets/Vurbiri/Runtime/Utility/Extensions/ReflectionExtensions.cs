...
namespace Vurbiri
{
    ...
    {
        [Impl(256)] public static Action<T> GetSetor<T>(this object self, string name)
        {
        ...
        [Impl(256)] public static Action<T> GetStaticSetor<T>(this Type self, string name)
        {
            var property = self.GetProperty(name);
            return (Action<T>)Delegate.CreateDelegate(typeof(Action<T>), null, property.GetSetMethod());
        }

        [Impl(256)] public static Func<T> GetGetor<T>(this object self, string name)
        {
            var property = self.GetType().GetProperty(name);
            return (Func<T>)Delegate.CreateDelegate(typeof(Func<T>), self, property.GetGetMethod());
        }
        [Impl(256)] public static Func<T> GetStaticGetor<T>(this Type self, string name)
        {
            var property = self.GetProperty(name);
        ...
        public static bool IsGeneric(this Type self, Type other)
        {
            while (self != null)
            {
                if (self.IsGenericType && self.GetGenericTypeDefinition() == other)
                    return true;
