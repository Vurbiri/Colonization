...
namespace Vurbiri.Colonization
{
	public class Forest : MonoBehaviour
	{
		...
		[SerializeField, Max(0.5f)] private RndMFloat _offsetRange; // = 0.3f;
		[Space]
		[SerializeField] private RndColor32 _colorRange;
		...
		{
			[SerializeField, MinMax(4, 7)] private RndInt _countVertexRange; // = new(5, 6);
			[Space]
			[SerializeField, Range(1f, 3f)] private float _heightBase; // = 1.55f;
			[SerializeField, Range(0.5f, 2f)] private float _radiusBase; // = 1.11f;
			[Space]
			[SerializeField, MinMax(0.4f, 1.5f)] private RndFloat _sizeRatioRange; // = new(0.65f, 1.15f);
			[SerializeField] private Chance _chanceSmall; // = 38;
			[Space]
			[SerializeField, Range(0.5f, 1f)] private float _ratioNextPos; // = 0.75f;
			[SerializeField, MinMax(0.4f, 0.9f)] private RndFloat _ratioNextSizeRange; // = new(0.68f, 0.72f);
			...
			public List<Triangle> Create(Vector3 position, Color32 color, Vector2 uv)
			{
				...
				void BranchCreate(int countBase, Vector3 position, Color32 color, Vector2 uv)
				{
					...
					float angle = RndFloat.Next(angleStep);
