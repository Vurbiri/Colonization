...
namespace Vurbiri.Colonization.UI
{
	sealed public class CurrentMaxAndProfit : ACurrentMax<ReactiveCombination<int, int, int, int>>
    {
        ...
        {
            _profitTMP.text = string.Format(PROFIT, active.ToStr(), passive.ToStr());
            _valueTMP.text = string.Format(COUNT, current.ToStr(), max.ToStr());
            _hintText = string.Format(_localizedText, current.ToStr(), max.ToStr(), active.ToStr(), passive.ToStr(), HEX.GATE.ToStr());
        }
