	...
	public partial class AIController
	{
		sealed private class Scientist : Counselor
		{
			...
			public Scientist(AIController parent) : base(parent)
			{
				...
				if (!(PerkTrees.IsPerkLearned(EconomicPerksId.Type, freePerks.economic) && PerkTrees.IsPerkLearned(MilitaryPerksId.Type, freePerks.military)))
				{
#if TEST_AI
					UnityEngine.Debug.Log($"[Scientist] {HumanId} FREE LEARN SUBSCRIBE");
#endif
					GameContainer.GameEvents.Subscribe(GameModeId.Play, FreeLearn);
				}

                // ============ Local =============
                void Create(int type, int freePerk)
				{
					var perkTree = PerkTrees; var leveling = _leveling; var perks = _perks;
					...
					if (!perkTree.IsAllTreeLearned(type))
					{
						...
						{
							if (id != freePerk && perkTree.GetNotLearned(type, id, out Perk perk))
							{
								if (perk.Level > level)
									leveling.Add(perk);
								else
			...
			{
				var freePerks = s_aiSettings.cheat.freePerks;
				if(turnQueue.currentId == HumanId && (turnQueue.turn + 2) >= (freePerks.turn + (HumanId << 1)))
				{
#if TEST_AI
